/**
 * Deploy the Simple Flash Loan Demo contract to a Midnight network
...
import { deriveOperatorSecret } from './flash-loan.logic';
...
// flash-loan contract keeps its private state empty ({}): everything secret
// flows through witnesses instead (trade, operator secret, division
// quotients).
const PRIVATE_STATE_ID = 'flashLoanPrivateState';

// The operator secret is derived deterministically from the wallet seed (see
// src/flash-loan.logic.ts) so deploy and withdrawProfit use the same 32 bytes.
// Only its SHA-256 commitment (the `operator` ledger field) is ever public.
...
// The operator secret is fixed at deploy time (constructor commitment) and
// reused by withdrawProfit. The trade witness is only exercised by
// executeFlashLoan (never by the constructor), so a placeholder is fine here.
const operatorSecret = deriveOperatorSecret(SEED);
let witnessTrade = { buyPrice: 0n, sellPrice: 0n, qty: 0n };

// The contract is loaded dynamically, so the compact-js types can't be
// inferred — treat the compiled-contract chain loosely (same as the
// reference scaffold).
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy simple-flash-loan-demo to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  if (unregisteredUtxos.length > 0) {
    ...
    // The registration tx must pay its own fee with already-generated dust.
    // Estimate the fee and wait for enough dust to be projected before
    // registering — otherwise WalletFacade throws "Insufficient generated dust
    ...
    await walletCtx.wallet.waitForGeneratedDust(unregisteredUtxos, fee, { timeoutMs: 600_000 });
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      // initialPrivateState. The flash-loan contract keeps an empty private
      // state ({}); all secrets flow through witnesses. args is the contract
      // constructor's arguments: empty for flash-loan's no-arg constructor.
